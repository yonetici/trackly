...
3. **wp-config Constant Active**: If `defined( 'TRACKLY_GA_JSON' )` is active, settings fields inside WordPress admin are ignored. Check your `wp-config.php` file structure.
...
2. **Sampling Rate**: If `trackly_sampling_rate` is set low (e.g., `10%` or `25%`), only a random fraction of visitor sessions record clicks. Raise it to `100%` during staging testing.
